...
def profile_required(view_func):
    """Decorator that redirects users to profile setup if their profile is incomplete."""
    def wrapper(request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            if created or not profile.phone or not profile.date_of_birth or not profile.national_id:
                messages.warning(request, 'Please complete your profile before applying for scholarships.')
